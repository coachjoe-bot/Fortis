...
// `prep` carries the Quick Log warm-up/cool-down booleans the same way — they
// stamp parsed_data.warmup_done/cooldown_done when the queued log finally lands.
export function queueOutbox(athleteId, text, { store, now, pure, note, prep } = {}) {
  ...
  const next = [...cur, { text: t, at: now || Date.now(), pure: !!pure, note: note ? String(note).slice(0, 1200) : null,
    prep: prep ? { warmup: !!prep.warmup, cooldown: !!prep.cooldown } : null }];

...
const GS = `@import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@300;400;500;600&display=swap'); *{box-sizing:border-box;margin:0;padding:0;} body{background:${C.navy};color:${C.text};font-family:'DM Sans',sans-serif;} input,textarea,select,button{font-family:'DM Sans',sans-serif;} ::-webkit-scrollbar{width:4px;} ::-webkit-scrollbar-track{background:${C.navy2};} ::-webkit-scrollbar-thumb{background:${C.border};border-radius:2px;} @keyframes fadeUp{from{opacity:0;transform:translateY(8px);}to{opacity:1;transform:translateY(0);}} @keyframes pulse{0%,100%{opacity:1;}50%{opacity:0.4;}} .fade-up{animation:fadeUp 0.25s ease forwards;}`;
...
// Check days since last log from Supabase – handled async below
...
(async()=>{
try {
const logs = await sbGet(“workouts”,`?athlete_id=eq.${athlete.id}&order=created_at.desc&limit=1`);
if(!logs||logs.length===0) {
// First time ever – no callout needed, greeting is fine
return;
}
...
```
    let callout = null;
    if(daysAgo>=7) {
      callout = `${athlete.name}. It's been ${daysAgo} days since your last workout log. That's a week. What happened? We can't build anything on inconsistency -- you know that. What did you get after today, and let's get back on track.`;
    } else if(daysAgo>=4) {
      callout = `${athlete.name}. ${daysAgo} days since your last log. I'm not going to lecture you -- just remember, it's not about workout 1. It's about workout 100. Consistency is what builds champions. What did you do today?`;
    } else if(daysAgo>=2) {
      callout = `Back at it, ${athlete.name}. What did you get after today?`;
    }
    // 0-1 days = normal greeting, no callout needed
    if(callout) {
      setMessages([{role:"assistant",content:callout}]);
    }
